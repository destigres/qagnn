...
class GoalOrientedQuestionDataset(Dataset):
    def __init__(
        self, edges_file, q_id_file, qa_map_file, emb_file, train_test_split=0.9
    ):
        ...
        self.total_edge_types = (
            max(
                [
                    np.max(self.f["most_damaging_edge_types"][i])
                    for i in range(self.q_ids.shape[0])
                ]
            )
            + 1
        )
        ...
        assert len(self.q_ids) == len(
            self.f["ground_truth_adj"]
        ), "Number of questions between edges dataset file and question_ids file does not match"
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            "og_graphs": og_sub_graphs,
            "pooled_representations": pooled_representations,
            "most_damage_edges": most_damaging_edges,
            "least_damage_edges": least_damaging_edges,
            "most_damage_edge_types": most_damaging_edge_types,
            "least_damage_edge_types": least_damaging_edge_types,
            "q_id": q_id,
            "question": self.q_dict[q_id]["question"],
            "answer": self.q_dict[q_id]["answerKey"],
            "choices": self.q_dict[q_id]["choices"],
        }
        return sample
    ...
    def link_prediction_iterator(self, train=True, train_samples_to_pick=-1):
        ...
        if train and train_samples_to_pick > -1:
            indices = set(
                random.sample(
                    range(1, self.train_edges[self.train_test_split] + 1),
                    k=train_samples_to_pick,
                )
            )
        else:
            indices = None
        ...
        for i in question_indices:
            ...
            for answer_index in range(5):
                ...
                # Iterate over most damaging edges
                for edge, edge_type in zip(
                    question["most_damage_edges"][answer_index],
                    question["most_damage_edge_types"][answer_index],
                ):
                    if edge[0] != -1 and edge[0] != 0 and edge[1] != 0:
                        count_valid_edges += 1
                        if indices == None or count_valid_edges in indices:
                            ...
                            inputs = np.concatenate(
                                [edge, pooled_representation, one_hot_r], axis=0
                            )
                ...
                # Iterate over least damaging edges
                for edge, edge_type in zip(
                    question["least_damage_edges"][answer_index],
                    question["least_damage_edge_types"][answer_index],
                ):
                    if edge[0] != -1 and edge[0] != 0 and edge[1] != 0:
                        count_valid_edges += 1
                        if indices == None or count_valid_edges in indices:
...
if __name__ == "__main__":
    ...
    total = 0
    for edge, edge_type in dev_ds.link_prediction_iterator(
        train=True, train_samples_to_pick=1000000
    ):
        total += 1
