...
def evaluate_accuracy(eval_set, model):
    ...
    with torch.no_grad():
        for q_ids, labels, *input_data in tqdm(eval_set):
            ...
            logits, _ = model(*input_data, q_ids=q_ids)
    ...
    with torch.no_grad():
        for q_ids, labels, *input_data in tqdm(eval_set):
            ...
            if mode == "incomplete":
                ...
                new_edge_index = []
                new_edge_type = []
                for batch_idx in range(len(edge_index)):  # each example per batch
                    batch_edge_index = []
                    batch_edge_type = []
                    set_to_drop = set(top_k[batch_idx]).union(set(bottom_k[batch_idx]))
                    for answer_idx in range(len(edge_index[batch_idx])):  # each answer
                        ...
                        batch_edge_index.append(edge_index_i[:, idx_to_keep])
                        batch_edge_type.append(edge_type_i[idx_to_keep])
                        pass
...
def eval_detail_custom(args, eval_mode, top_x_percent):
    """
